...
class DRIFTLLM(PromptingLLM):
    ...
    def _aa_enabled(self):
        return getattr(self.args, "enable_argument_authority_drift", False)

    def _aa_load_global_contract(self):
        if not self._aa_enabled():
            return None
        if self.aa_global_contract is None:
            self.aa_global_contract = load_global_contract(self.args.aa_global_contract_path)
        return self.aa_global_contract

    def _aa_global_contract_subset(self, trajectory):
        global_contract = self._aa_load_global_contract() or {}
        return {
            tool_name: global_contract[tool_name]
            for tool_name in trajectory
            if tool_name in global_contract
        }

    def _aa_tool_schemas_for_trajectory(self, trajectory):
        trajectory_set = set(trajectory)
        return [
            tool_schema
            for tool_schema in getattr(self, "tools_docs_list", [])
            if tool_schema.get("name") in trajectory_set
        ]

    def _aa_build_argument_contract(self, query):
        if not self._aa_enabled() or self.aa_argument_contract is not None:
            return
        self._aa_load_global_contract()
        if self.aa_provenance_store is None:
            self.aa_provenance_store = ProvenanceStore()
        self.aa_provenance_store.record_user_explicit(query)

        trajectory = list(self.function_trajectory)
        contract_model = self.args.aa_contract_model or self.args.model
        global_subset = self._aa_global_contract_subset(trajectory)
        tool_schemas = self._aa_tool_schemas_for_trajectory(trajectory)
        self.aa_argument_contract = build_argument_contract(
            client=self.client,
            model=contract_model,
            user_query=query,
            trajectory=trajectory,
            tool_schemas=tool_schemas,
            global_contract_subset=global_subset,
        )
        if getattr(self.args, "aa_debug", False):
            self.logger.info(f"AA Argument Contract: {self.aa_argument_contract}")

    def aa_result_fields(self):
        events = list(self.aa_validation_events)
        reason_counts = {}
        for event in events:
            if event.get("passed"):
                continue
        ...
        return {
            "argument_authority_enabled": self._aa_enabled(),
            "aa_global_contract_path": getattr(self.args, "aa_global_contract_path", None),
            ...
            "aa_decision_summary": {
                ...
                "num_source_not_authorized": reason_counts.get("source_not_authorized", 0),
                "num_required_proof_missing": reason_counts.get("required_proof_missing", 0),
                "num_deny_mark_hit": reason_counts.get("deny_mark_hit", 0),
            },
        }
    ...
    def argument_authority_validation(self, json_tool_calls, output, query, messages):
        """Validate action arguments against the AA-DRIFT contract and provenance."""
        align_error_message = None
        if self.aa_argument_contract is None:
            self._aa_build_argument_contract(query)
        if self.aa_provenance_store is None:
            self.aa_provenance_store = ProvenanceStore()
        ...
        for call in json_tool_calls:
            ...
            ok, reason, events = validate_action_arguments(
                tool_name=tool_name,
                arguments=arguments,
                argument_contract=self.aa_argument_contract or {},
                provenance_store=self.aa_provenance_store,
                global_contract=self.aa_global_contract or {},
            )
            self.aa_validation_events.extend(events)
            if ok:
                continue
            ...
            align_error_message = {
                "role": "user",
                "content": (
                    "[CALL ERROR] The function call was refused because one or more arguments do not satisfy "
                    "the argument authority contract.\n"
                    f"Rejected sink: {event.get('sink', tool_name)}\n"
                    f"Allowed sources: {event.get('allowed_sources', [])}\n"
                    f"Actual sources: {event.get('actual_sources', [])}\n"
                    f"Reason: {reason}\n"
                    "Please use only task-authorized data sources."
                ),
            }
            self.logger.info(f"AA-DRIFT refused {call}: {align_error_message}")
            output["tool_calls"] = []
            return align_error_message, output
    ...
    def query(
        ...
        if self._aa_enabled():
            ...
            if self.aa_provenance_store is None:
                self.aa_provenance_store = ProvenanceStore()
                self.aa_provenance_store.record_user_explicit(query)
            self.aa_provenance_store.record_new_tool_outputs(messages, self.function_trajectory)
        ...
        if self._aa_enabled() and self.args.build_constraints and self.function_trajectory:
            self._aa_build_argument_contract(query)
        ...
        execution_checklist = self.node_checklist
        if self._aa_enabled() and self.aa_argument_contract is not None:
            execution_checklist = summarize_argument_contract(self.aa_argument_contract)
        completion = self.client.agent_run(openai_messages, self.tools_docs_list, query=query, initial_trajectory=self.function_trajectory, achieved_trajectory=self.achieved_function_trajectory, node_checklist=execution_checklist)
        ...
        for _ in range(self._MAX_ATTEMPTS):
            try:
                ...
                completion = self.client.agent_run([*openai_messages, self._message_to_sharegpt(error_message)], self.tools_docs_list, query=query, initial_trajectory=self.function_trajectory, achieved_trajectory=self.achieved_function_trajectory, node_checklist=execution_checklist)
